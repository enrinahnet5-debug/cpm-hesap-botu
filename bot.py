...
def update_user_balance(user_id, points, refs):
    users = []
    found = False
    if os.path.exists(USER_DATA_FILE):
        with open(USER_DATA_FILE, "r", encoding="utf-8") as f:
            for line in f:
                ...
                uid = int(parts[0])
                if uid == user_id:
                    users.append(f"{user_id},{points},{';'.join(map(str, refs))}\n")
                    found = True
                else:
                    users.append(line)
    if not found:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    points, refs = get_user_balance(user.id)
    ...
    if args and args[0].isdigit():
        ...
        if ref_id != user.id and ref_id not in refs:
            refs.add(ref_id)
            update_user_balance(user.id, points, refs)
            ref_points, ref_refs = get_user_balance(ref_id)
            update_user_balance(ref_id, ref_points + 5, ref_refs)
            try:
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "profile":
        ...
        account = stock.pop(0)
        save_stock(stock)
        update_user_balance(user.id, points - 15, refs)
